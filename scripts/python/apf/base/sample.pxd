...
from apf.base.cyutils cimport _logsumexp
...
cdef extern from "gsl/gsl_randist.h" nogil:
    ...
    unsigned long int gsl_rng_uniform_int(const gsl_rng * r, int n)
...
cdef inline int _sample_logcategorical(gsl_rng * rng, double[::1] logdist) nogil:
...
cdef class Sampler:
    ...
    cpdef int logcategorical(self, double[::1] logdist)
