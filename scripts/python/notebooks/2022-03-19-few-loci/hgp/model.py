...
datafile_ls = glob.glob("data/adam/*.csv")
...
for datafile in datafile_ls:
	...
	df_data = df_data.sort_values(by=['Condition_1']).reset_index(drop=True)
	...
	print(data)
	datafile = datafile[10:][:-4] + '_'
	print(datafile)
	# break
	# temp_path = datafile[16:][:-14] + datafile[16:][-4:]
	temp = np.load("data/adam/Caulo_unique_processed.csv_threshold.npy")
	...
	n_biopsies_I = np.asarray(df_data.groupby(['Condition_1']).size()) ### this sort by alphabeta
	...
	row = ['H'] + ['Top_Level'] + \
		  list(df_data.groupby(['Condition_1']).groups.keys()) + \
		  list(df_data.groupby(['Condition_1', 'Index']).groups.keys())
